...
```{r solar}
source("/Users/javipatron/Documents/MEDS/Courses/eds230/classes/ESM232_Examples/R/solarpv.R")


# read in R formatted data
load("/Users/javipatron/Documents/MEDS/Courses/eds230/classes/ESM232_Examples/Data/sierraczosolar.rda")
...
almond_model <- function(file, parm1 = -0.015, parm2 = -0.0046, parm3 = -0.07, parm4 = 0.0043, parm5 = 0.28){
  ...
  slice(-1) |> 
    ...
    mutate(profit = 100000 + anomaly * 100000)
  

  # Return the final result which is a list of two objects. A data set and the mean of that data set.
  return(list(annual = clim_df[,c("anomaly","profit")], 
              mean = mean(clim_df$profit)))
...
# generate samples for both parameters
nsamples = 10
ethresh = runif(min = min(clim_df$precip),
                max = min(clim_df$precip), 
                n = nsamples) # Use random uniform if you have no clue how they created the coefficnet variable.

parm1 = rnorm(mean = -0.015, sd = 0.01, n = nsamples)
parm2 = rnorm(mean = -0.048, sd = 0.05, n = nsamples)
...
Use the function created to build the new values and plot what you need.
```{r}
...
# and we can add the parameter values for each run
mean_profit_df = cbind.data.frame(mean_profit, parms)

# plot - pick on of the 2 parameter as a color

ggplot(mean_profit_df, aes(x = parm1, y = mean, col = parm2)) +
  geom_point() +
  labs(y="Mean Annual Electricity W", 
       x="Threshold Radiation (kJ/m2)  \n above which energy production is more efficient")

ggplot(mean_profit_df, aes(parm2, mean, col = parm1))+geom_point(cex=2)+
  labs(y="Mean Annual Electricity W", x="Efficiency")
ggarrange(p1,p2)
